...
export function UsageTrendChart({
  ...
  const slotWidth = Math.max(
    ...
    Math.floor(CHART_MIN_WIDTH / Math.max(points.length, 1))
  );
  const chartWidth = Math.max(
    CHART_MIN_WIDTH,
    points.length * slotWidth + CHART_PADDING_X * 2
  );
    ...
    <section className="card usage-chart-card" aria-labelledby="usage-trend-title">
      ...
      {hasUsage ? (
        <div className="usage-chart">
          <div className="usage-chart__viewport">
            <svg
            ...
            >
              ...
              {points.map((point, index) => {
                const labelX = CHART_PADDING_X + index * slotWidth + slotWidth / 2;
                const x = labelX - CHART_BAR_WIDTH - CHART_BAR_GAP / 2;
                const inputHeight =
