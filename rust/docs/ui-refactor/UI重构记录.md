...
| 2026-07-31 18:35 +08:00 | 范围外 / 后端独立修复 | `src-tauri/src/claude.rs`（commit `fc1b001`） | **范围外说明**：`validate_work_dir` 剥离 Windows `\\?\` 扩展长度前缀以解决 cmd `cd /d "{work}"` 将 `\\?\C:\...` 当作 UNC 路径拒绝的问题；附回归测试 `validate_work_dir_strips_verbatim_prefix` / `validate_work_dir_rejects_missing_dir`。该提交**不在 UI 重构范围**（`UI重构Goal.md` 约束 1 + 实施计划均要求「不修改 `src-tauri/src/**`」），是 UI 重构期间独立发现的 user-flow bug 修复，因此单独提交；剥离逻辑发生在已校验 canonical 路径之后，无新注入向量 | `cargo fmt --check` 通过；剥离逻辑正确（`\\?\UNC\` → `\\`，`\\?\` → 裸）；回归测试覆盖前缀剥离 + 不存在目录拒绝；其余安全闸门（`logger.rs`/`nvidia/*`/`config.rs`/`history.rs`/`lib.rs`/`tauri.conf.json`）未触 | 该修改未走 UI 评审；如要严格遵守「范围外=禁止」原则，回滚方式 = `git revert fc1b001`，代价是选择工作目录后点「启动 Claude Code」会再次触发 `CMD 不支持将 UNC 路径作为当前目录` |
