...
| A-02 | 阻断 | React.lazy chunk 失败没有 Error Boundary | `src/App.tsx` | 增加页面级错误边界与恢复操作 | 待处理 | 单测模拟 lazy 失败；重试后恢复 |
...
| A-14 | 功能风险 | 重要输入 label 未绑定字段 | Config/NVIDIA/Launch/Proxy | 使用稳定 id、`htmlFor`、`aria-describedby`、错误关联 | 待处理 | `getByRole` 按名称定位；键盘/读屏验证 |
...
| ID | 优先级 | 原问题 | 文件/组件位置 | 重构方案 | 状态 | 验证证据 |
|---|---|---|---|---|---|---|
| V-01 | 高 | 没有深色模式，背景和表面大量写死 | `styles.css`、`index.html`、多个 TSX inline style | 语义 Token + `prefers-color-scheme`；启动屏同步 | 待处理 | 浅/深主题状态矩阵与截图 |
...
| V-04 | 高 | 全站 `:focus-visible` 不统一 | Sidebar、按钮、tab、列表、输入 | 全局可见 focus ring；不以 hover 替代 | 待处理 | 全站 Tab 顺序检查 |
| V-05 | 高 | 侧栏蓝紫渐变、进度条渐变违反目标 | `Sidebar.tsx`、`.dict-progress-fill` | 使用单一系统色和语义状态色 | 待处理 | `rg` 无 `gradient`；视觉检查 |
...
| V-12 | 中 | 毛玻璃无统一降级主题 | topbar/sidebar/modal | 仅导航与模态遮罩使用 glass Token，支持 reduced-transparency | 待处理 | media query 与深色检查 |
| V-13 | 中 | 启动屏用 emoji logo，颜色写死 | `index.html` | 使用真实应用图标或纯文字系统启动状态，跟随主题 | 待处理 | 启动阶段浅/深色检查 |
