...
- [x] **Step 1: Add `mock_stream_error_then_success`**
...
- [x] **Step 2: Add `body_decoding_error_before_handoff_retries_the_request`**
...
The test was compiled before the production change. Runtime execution is blocked in this Windows environment by the existing `STATUS_ENTRYPOINT_NOT_FOUND (0xc0000139)` DLL startup failure, so the expected pre-fix assertion failure could not be observed in the test process.
...
- [x] **Step 1: Track completion separately from meaningful output**
...
- [x] **Step 2: Change `wait_for_meaningful_stream_start` into a bounded prefetch**
...
if incomplete_eof_before_completion {
    return StreamStart::Failed("upstream stream ended before completion".to_string());
}
...
Use a first-output deadline before meaningful output and the post-output handoff deadline afterward; do not apply the first-output timeout to the entire generation. A stream ending without meaningful output remains `Ended`, `Idle`, or `BufferLimitExceeded` as before so existing model fallback behavior is preserved. A stream that produced meaningful output but ended before completion uses `Failed` and the existing retry/key-rotation path.

- [x] **Step 3: Keep the existing retry and post-handoff behavior**
...
- [x] **Step 1: Make the partial-output fixture cross the handoff window**
...
- [x] **Step 2: Preserve the safety assertions**
...
- [x] **Step 1: Run the focused regression test and existing proxy tests**

```bash
cargo test --lib nvidia::proxy::stream_stall_fallback_tests -- --nocapture
```

The test binary compiles, but runtime execution is blocked by `STATUS_ENTRYPOINT_NOT_FOUND (0xc0000139)` before the harness starts.

- [x] **Step 2: Run Rust formatting and compile checks**
...
Result: `cargo fmt --all -- --check`, `cargo check --tests`, and `cargo clippy --all-targets -- -D warnings` pass.

- [x] **Step 3: Run the complete Rust unit suite**

```bash
cargo test --lib
```

Result: test execution is blocked by `STATUS_ENTRYPOINT_NOT_FOUND (0xc0000139)`; `cargo test --lib --no-run` passes and produces the test executable.
...
- [x] **Step 1: Create the implementation commit**
