// 无头端到端冒烟：验证「8083（通用协议网关）的请求 → UsageStatsStore → UI 面板快照」
// 这条统计链路真的通。
//
// 不需要真实 provider Key：本地拉起一个 mock OpenAI 上游（/v1/chat/completions），
// 把 ProviderEntry.base_url 指向它，然后经真实监听端口发两条 Anthropic 请求
// （一条非流式、一条流式），最后读 UsageStatsStore::snapshot —— 这正是 Tauri 命令
// get_usage_stats 返回给前端 DashboardPage 的同一份数据。
//
// 运行：cargo run --example gateway_stats_smoke
...
use claude_launcher_lib::gateway::config::{AuthMode, ProtocolKind, ProviderEntry};
...
const UPSTREAM_MODEL: &str = "mock-model";
...
async fn run() -> bool {
    ...
    let dir = std::env::temp_dir().join(format!(
        "claude-launcher-gw-stats-smoke-{}",
        std::process::id()
    ));
    ...
    // 3. 拉起 8083 网关（用随机空闲端口，避免和用户真实运行中的 8083 打架）
    let port = free_port();
    ...
    match state.start(provider, Arc::clone(&store)) {
        Ok(msg) => println!("{msg}"),
        Err(e) => {
            println!("FAIL 启动网关: {e}");
            return false;
    ...
    // 4a. 非流式请求（Claude Code 侧发的是 claude-sonnet-4，网关经 model_map 回落到 models[0]）
    let resp = client
        .post(&endpoint)
        .json(&json!({
    ...
    for (label, range) in [
        ...
        let gw = snap.providers.iter().find(|p| p.provider == "deepseek");
        all_ok &= check(
            &format!("[{label}] providers 含 deepseek（网关 provider.id）"),
            gw.is_some(),
            format!(
                "providers={:?}",
                snap.providers
        ...
        if let Some(p) = gw {
            all_ok &= check(
                &format!("[{label}] deepseek 请求数=2"),
                p.requests == 2,
                format!("requests={}", p.requests),
            );
            all_ok &= check(
                &format!("[{label}] deepseek token 合计={expect_total}"),
        ...
        let model_row = snap
            .models
            .iter()
            .find(|m| m.provider == "deepseek" && m.model == UPSTREAM_MODEL);
        all_ok &= check(
            &format!("[{label}] 模型明细含 deepseek/mock-model"),
    ...
    all_ok &= check(
        "usage-stats.json 已落盘且含 deepseek",
        on_disk.contains("deepseek") && on_disk.contains(UPSTREAM_MODEL),
