//! MCP stdio 客户端 + `McpTool`(`impl Tool`)。
//!
//! 这一模块是 P8 最重的一块。MCP JSON-RPC 2.0 极窄面客户端 —— 手写,不引 crate。
//! `RpcEnvelope` / `RpcError` 几个 serde struct + 逐行 `serde_json` 往返;`McpClient` 封装一
//! server 子进程的 stdin/stdout,后台 read task 把每行解包后按 `id` 扇回到 `request` 的 oneshot;
//! `McpTool` 把一远端工具包成 `impl Tool`,`execute` 直接 `.await` `call_tool`(async,经
//! `#[async_trait]` 分派)。
//!
//! **(a) 前**:`Tool::execute` 曾为同步 `&self` 签名,async 子进程 IO 靠本模块的 `(c′) 桥
//! block_on_current`(独立 OS 线程 + 独立 multi_thread runtime 借跑同步 execute)。**P9 (a) trait
//! async 升级后桥已整段删除** —— `Tool::execute` 升 `async fn` + `#[async_trait]`,McpTool/Bash/
//! SubagentTool 直接在自己的生产 runtime 上 `.await`,回常规 tokio 形态。桥 + 4 个析因消融测试 +
//! 桥-timer-fires 测试已删;桥上 timer-stall 那段硬坑(§14.4)的实证数字 59.62s/59.95s/2.11s/7.13s
//! 永久留在 journey §14.4,此处只记「已删」。
//!
//! 诚实边界:握手 / list_tools / call_tool 的真往返要真 MCP server 才能验 —— `tests/mcp_fake_handshake.rs`
//! 是无 key、CI 自跑的 fake-server 全链(Phase A 基线经桥绿、Phase B 无桥再绿,证 async 路径运行期等价);
//! 真 `npx` PATHEXT + 冷拉握手见 `.e2e/t01/t02`(手动,留本机)。纯函数 / 配置解析层已单测焊(见
//! config::tests 与本模块 `resolve_program_*`/`resolve_in_paths_*` 测试)。
...
impl Tool for McpTool {
    ...
    async fn execute(&self, arguments: &str) -> anyhow::Result<String> {
        ...
        // (a) Phase B:async 化后直接 await,不再经 (c′) 桥 block_on_current 借独立 OS 线程 +
        // 临时 runtime —— 桥已整段删除。运行期等价由 `tests/mcp_fake_handshake.rs`(无 key、
        // CI 自跑真 McpClient::spawn → handshake → list_tools → call_tool 全链,Phase A 基线
        // 经桥绿、Phase B 无桥再绿)证。
        // 用 remote_name(server 原名)发 tools/call,不是带 prefix 的 self.name。
        let remote = self.remote_name.clone();
        let res = self.client.lock().await.call_tool(&remote, args).await;
